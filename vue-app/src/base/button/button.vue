<template>
  <button
    :type="nativeType"
    class="newb-button"
    :class="['newb-button--' + type, 'newb-button--' + size, {
    ...
    <span class="newb-button-icon" v-if="icon || $slots.icon">
      <slot name="icon">
        <i v-if="icon" class="newbui" :class="'newbui-' + icon"></i>
      </slot>
    </span>
    <label class="newb-button-text"><slot></slot></label>
  </button>
</template>
<script>
  /**
